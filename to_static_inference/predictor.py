...
class Benchmark(object):
    ...
    def set_config(self, **options):
        ...
        device_id = options.get("device_id", 4)
    ...
    def set_data_shape(self, shapes, dtypes=None):
        """
        load_data
        """
        raise NotImplementedError

    def run(self, warmup=10, repeat=10):
        # step 1: prepare data
        input_names = self.predictor.get_input_names()
        for i, input_data_name in enumerate(input_names):
            input_tensor = self.predictor.get_input_tensor(input_data_name)
            i_shape = self.shapes[i]
            i_dtype = self.dtypes[i]
            input_tensor.reshape(i_shape)
            np_data = np.ones(i_shape).astype(i_dtype)
        ...
        for i in range(repeat):
            ...
            for i, output_data_name in enumerate(output_names):
                output_tensor = self.predictor.get_input_tensor(output_data_name)
                output = output_tensor.copy_to_cpu()
                outputs.append(output)
            # finish whole process of prediction
            t2 = time.time()
            cost_time.append((t2 - t1)*1000)
